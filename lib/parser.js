...
const parse = (chunk, offset, printParsed, trackPtr) => {
    ...
    for (let i = 0; i < 8; i += 1) {
        ...
        while (cont && (chunk[cmdPtr] !== 0 || cmdPtr < trackEntry[i + 1])) {
            ...
            if (chunk[cmdPtr] >= 0x1 && chunk[cmdPtr] <= 0x7F) {
                ...
                sequenceData.push({ addr: comPtrPrev - offset, data });
                parselogger.add(`${trace16(comPtrPrev - offset)}: ${printableBuffer(data)}`);
                cmdPtr += 1;
            } else if (chunk[cmdPtr] >= 0x80 && chunk[cmdPtr] <= 0xDF) {
                //
                // 0x80 - 0xDF: Notes, Tie, Rest, Percussion Note
                //
                sequenceData.push({ addr: comPtrPrev - offset, data: [chunk[cmdPtr]] });
                parselogger.add(`${trace16(comPtrPrev - offset)}: ${trace8(chunk[cmdPtr])}`);
                cmdPtr += 1;
                ...
                switch (data[1]) {
                    ...
                    case 0x7: {
                        ...
                        if (mentionedAddr.indexOf(target - offset) < 0) {
                            mentionedAddr.push(target - offset);
                        }
                        if (target > cmdPtr) {
                            cmdPtr = target;
                        } else {
    ...
    return { trackData, mentionedAddr };
};

 ...
 */
const convAMK = (chunk, offset, ast, mentionedAddr, instPtr, amkFix, barebones, doubleTick) => {
    let mml = '';
    const usedInst = [];
    const rmcs = [];
    const brrChunks = {};
        ...
        for (let i = 0; i < currentTrack.length; i += 1) {
            ...
            if (first >= 0x1 && first <= 0x7F) {
                ...
                switch (first) {
                    ...
                    case 0xE7: {
                        const tempo = doubleTick ? data[1] * doubleTick : data[1];
                        mml += `t${tempo}\t; About ${Math.round(tempo / 0.4096)} BPM\n`;
                        break;
                    }
                    case 0xED: {
                        mml += `v${data[1]} `;
